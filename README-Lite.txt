...
- 未安装时 Start-Practice.bat 会提示；也可改用完整便携版 XinDianPrac-Portable-*（自带运行时，无需安装任何东西）。
...
目录说明
- dist\      控制台核心（XinDianPrac.exe / XinDianPrac.dll），运行记录写入 dist\logs
- dist-ui\   界面（XinDianPrac.Gui.dll）
- config\    entrances.json（Boss 入口表）、settings.json（开局默认值）、game-target.json（游戏路径）
- config\ 和 dist\config\ 都要保留：核心是按自身目录读取 dist\config\entrances.json 的
